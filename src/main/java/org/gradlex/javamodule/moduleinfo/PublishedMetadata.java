...
public class PublishedMetadata implements Serializable {
    ...
    private List<String> componentVariant(
        ...
        versionsSource.getAttributes().keySet().forEach(a -> {
            ...
            singleComponentVariantResolver
                    ...
                    .attribute(
                            untypedAttributeKey,
                            requireNonNull(versionsSource.getAttributes().getAttribute(a)));
        });
        singleComponentVariantResolver
                .getAttributes()
                .attribute(USAGE_ATTRIBUTE, project.getObjects().named(Usage.class, usage));
    ...
    private Configuration maybeCreateDefaultVersionSourcConfiguration(
            ConfigurationContainer configurations, ObjectFactory objects, @Nullable SourceSetContainer sourceSets) {
            ...
            if (sourceSets != null) {
                for (SourceSet sourceSet : sourceSets) {
                    Configuration implementation = configurations
                            ...
                            .get();
                    Configuration compileOnly = configurations
                            .named(sourceSet.getCompileOnlyConfigurationName())
                            .get();
                    Configuration runtimeOnly = configurations
                            .named(sourceSet.getRuntimeOnlyConfigurationName())
                            .get();
                    Configuration annotationProcessor = configurations
    ...
    private List<String> firstAndOnlyComponentDependencies(Configuration singleComponentVariantResolver) {
        ...
        if (result instanceof UnresolvedDependencyResult) {
            ...
            return ((ResolvedDependencyResult) result)
                    .getSelected().getDependencies().stream()
    ...
    private static boolean filterComponentDependencies(DependencyResult d) {
        if (d instanceof ResolvedDependencyResult) {
            Category category = ((ResolvedDependencyResult) d)
                    .getResolvedVariant()
                    .getAttributes()
                    .getAttribute(CATEGORY_ATTRIBUTE);
            String categoryUntyped = ((ResolvedDependencyResult) d)
                    .getResolvedVariant()
                    .getAttributes()
                    .getAttribute(CATEGORY_ATTRIBUTE_UNTYPED);
    ...
    private static String ga(DependencyResult d) {
        if (d instanceof ResolvedDependencyResult) {
            return ExtraJavaModuleInfoPlugin.ga(
                    ((ResolvedDependencyResult) d).getSelected().getId());
        }
