...
class FilePathToModuleCoordinatesTest extends Specification {
    ...
    def "ga coordinates from gradle cache file path"() {
        ...
        expect:
        gaCoordinatesFromFilePathMatch(path, "org.slf4j:slf4j-api", null)
    }
    ...
    def "ga coordinates from gradle cache file path (version in file name does not match)"() {
        ...
        expect:
        gaCoordinatesFromFilePathMatch(path, "com.google.guava:guava", null)
    }

    def "ga coordinates from gradle cache file path with classifier"() {
        given:
    ...
    def "ga coordinates from m2 repo file path"() {
        ...
        then:
        gaCoordinatesFromFilePathMatch(jarPath, "com.google.code.findbugs:jsr305", null)

        when:
        jarPath = path('/Users/someone/.m2/repository/de/odysseus/juel/juel-impl/2.2.7/juel-impl-2.2.7.jar')

        then:
        gaCoordinatesFromFilePathMatch(jarPath, "de.odysseus.juel:juel-impl", null)
    }
    ...
    def "ga coordinates from m2 repo file path with classifier"() {
        given:
